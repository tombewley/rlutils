...
class DynamicsModel:
    ...
    def plan(self, state_init, policy, ensemble_index):
        """
        ...
        batch_size, action_dim = state_init.shape[0], self.action_space_low.shape[0]
        states  = torch.empty((self.P["num_iterations"], self.P["num_particles"], self.P["horizon"]+1, batch_size, state_init.shape[1]), device=state_init.device)
        actions = torch.empty((self.P["num_iterations"], self.P["num_particles"], self.P["horizon"],   batch_size, action_dim         ), device=state_init.device)
        returns = torch.zeros((self.P["num_iterations"], self.P["num_particles"], self.P["horizon"],   batch_size                     ), device=state_init.device)
        # Initiate state at t = 0 and mean/std at i = 0.
        states[:,:,0] = state_init

        assert policy == "cem" or type(policy) == SequentialNetwork # TODO: Make more generic
        if policy == "cem":
            assert batch_size == 1
            #### TODO: ####
            mean    = torch.empty((self.P["num_iterations"],                          self.P["horizon"],   batch_size, action_dim         ), device=state_init.device)
            std     = torch.empty((self.P["num_iterations"],                          self.P["horizon"],   batch_size, action_dim         ), device=state_init.device)
            mean[0] = torch.zeros((self.P["horizon"], batch_size, action_dim))
            std[0] = 2*torch.ones((self.P["horizon"], batch_size, action_dim))
            ###############
        ...
        for i in range(self.P["num_iterations"]):
            ...
            for t in range(self.P["horizon"]):
                if type(policy) == SequentialNetwork: 
                    actions[i,:,t] = self._action_scale(policy(states[i,:,t]))
                states[i,:,t+1] = self.predict(states[i,:,t], actions[i,:,t], ensemble_index)
                returns[i,:,t]  = returns[i,:,t-1] + (self.P["gamma"] ** t) * self.reward_function(states[i,:,t], actions[i,:,t], states[i,:,t+1])
    ...
    def _action_scale(self, actions):
        ...
        NOTE: duplicated from agents._generic but cleaner to avoid reference to agent."""
